...
function getWingetFlags() {
    ...
    if (DOM.toggles.disableInteract.checked) flags.push("--disable-interactivity");
...
function applyFilters() {
    ...
    const filtered = state.apps.filter(app => {
        const searchStr = `${app.name} ${app.description} ${app.packagename} ${app.category}`.toLowerCase();
...
DOM.clearBtn.addEventListener("click", () => {
    if (state.selectedPackages.size === 0) return showToast("List is already empty", "info");
...
DOM.copyAllBtn.addEventListener("click", async () => {
    if (state.selectedPackages.size === 0) return showToast("No apps selected", "error");
    
    const commands = [...state.selectedPackages].map(pkg => generateInstallString(pkg)).join("\n");
...
DOM.schemaBtn.addEventListener("click", () => {
    ...
    const schema = {
        ...
        "Sources": [{
            "Packages": [...state.selectedPackages].map(pkg => ({ "PackageIdentifier": pkg })),
            "SourceDetails": {
    ...
    downloadFile("winget-schema.json", JSON.stringify(schema, null, 2), "application/json");
...
DOM.exportPs1Btn.addEventListener("click", () => {
    ...
    [...state.selectedPackages].forEach(pkg => {
        ...
        scriptLines.push(`if ($LASTEXITCODE -ne 0) { Write-Host 'Warning: Install failed for ${pkg}' -ForegroundColor Yellow }`);
        scriptLines.push("");
    });

    scriptLines.push("Write-Host 'Installations Complete!' -ForegroundColor Green");
